...
static const unsigned long crc32b_table[256] =
{
...
unsigned long fast_crc32b_array(const unsigned char * data, size_t len)
{
    ...
    while (len--) {
        crc = (crc >> 8) ^ crc32b_table[(crc ^ *data++) & 0xFF];
    }
...
unsigned long fast_crc32b_byte(const unsigned long crc, const unsigned char data) // must ~crc if last byte
{
