...
// TUPLE((entityName, descr, isCustomSpace, isHeap, arg), (type1, name1, bitFlags1, descr1, incrementSize1, fieldInitEnable1, arrayStartNumber1, specSymbol1) ... (typeN, nameN, bitFlagsN, descrN, incrementSizeN, fieldInitEnableN, arrayStartNumberN, specSymbolN))
// parameter: (breakExpr, entityNumber, fieldNumber, entityPtr, structObjectPointer, structName)

#define ENTITY_STRUCT_MAKE_VAR(x) PREPROCESSOR_IF( PREPROCESSOR_PRIMITIVE_NOT(PREPROCESSOR_NAT_EQ(PREPROCESSOR_TUPLE_COUNT(x), 7))) (PREPROCESSOR_GET_ARG(7)(PREPROCESSOR_UNTUPLE(x))) PREPROCESSOR_GET_ARG(0)(PREPROCESSOR_UNTUPLE(x)) PREPROCESSOR_GET_ARG(1)(PREPROCESSOR_UNTUPLE(x))   /* for making struct fields (auto adds [] if this field is array)*/  \
