 ...
 * TO TXT EXPRESSIONS
 */
#define TO_TXT(X) TO_TXT1024(X)
#define TO_TXT1024(X) TO_TXT512(TO_TXT512(X))
#define TO_TXT512(X) TO_TXT256(TO_TXT256(X))
#define TO_TXT256(X) TO_TXT128(TO_TXT128(X))
#define TO_TXT128(X) TO_TXT64(TO_TXT64(X))
#define TO_TXT64(X) TO_TXT32(TO_TXT32(X))
#define TO_TXT32(X) TO_TXT16(TO_TXT16(X))
#define TO_TXT16(X) TO_TXT8(TO_TXT8(X))
#define TO_TXT8(X) TO_TXT4(TO_TXT4(X))
#define TO_TXT4(X) TO_TXT2(TO_TXT2(X))
...
#define DEFER4(...) __VA_ARGS__ EMPTY EMPTY EMPTY EMPTY()()()()
#define DEFER5(...) __VA_ARGS__ EMPTY EMPTY EMPTY EMPTY EMPTY()()()()()
 ...
 * inputs must not.
 *
 * IS_PROBE(PROBE())    // Expands to 1
 * IS_PROBE(xxx)        // Expands to 0
 */
 ...
 *
 *      COMPL(0) // Expands to 1
 *      COMPL(1) // Expands to 0
 */
...
/************************************************************************************************************************************************************
 * MAP FOR MERGING NAMES TO ONE NAME
 * Example Usage:
 *      MAP_ALL_TO_ONE(sum,__,int, float, double) // __ - is separator
 *
 * Which expands to:
 *      sum__int__float__double
 */

#define MAP_ALL_TO_ONE(...) \
   IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_ALL_TO_ONE_INNER(__VA_ARGS__)))
#define MAP_ALL_TO_ONE_INNER(op,sep,cur_val, ...) \
    IF_ELSE(HAS_ARGS(__VA_ARGS__))( \
        DEFER2(_MAP_ALL_TO_ONE_INNER)()(CAT3(op,sep,cur_val),sep, __VA_ARGS__), \
        CAT3(op,sep,cur_val)\
    )
#define _MAP_ALL_TO_ONE_INNER() MAP_ALL_TO_ONE_INNER
 ...
 */
#define REPEAT_MACRO(...) \
    IF(HAS_ARGS(__VA_ARGS__))(EVAL(REPEAT_MACRO_INNER(__VA_ARGS__)))
#define REPEAT_MACRO_INNER(count, macro, ...) \
    IF(count) ( \
        DEFER2(_REPEAT_MACRO_INNER) () (MAP_DEC(count), macro, ##__VA_ARGS__) \
        DEFER2(macro) (MAP_DEC(count), ##__VA_ARGS__) \
    )
#define _REPEAT_MACRO_INNER() REPEAT_MACRO_INNER
 ...
 */
#define WHILE_MACRO(...) \
    IF(HAS_ARGS(__VA_ARGS__))(EVAL(WHILE_MACRO_INNER(__VA_ARGS__)))
#define WHILE_MACRO_INNER(pred, op, ...)\
    IF_ELSE(pred(__VA_ARGS__)) ( \
        DEFER2(_WHILE_MACRO_INNER) () (pred, op, op(__VA_ARGS__)), \
        __VA_ARGS__ \
    )
#define _WHILE_MACRO_INNER() WHILE_MACRO_INNER
